...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_past_questions(self):
        ...
        future_question=Question(question_text="question text", pub_date=timezone.now())
        self.assertIs(future_question.was_published_recently(),True)
    
    def test_was_published_recently_with_present_questions(self):
        """was_published_recently returns True for questions whose pub_date is in the future"""
        time=timezone.now()+datetime.timedelta(days=-30)
        future_question=Question(question_text="question text", pub_date=time)
        self.assertIs(future_question.was_published_recently(),False)
...
def create_question(question_text,days):
    """
    Creates a question with the given "question_text" and number of days offset to now
    (negative for questions published in the past,
    positive for questions that have not been published yet)
    """
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_past_questions(self):
        """                
        Questions with apub_date in the past are displayed in the index page
        """
